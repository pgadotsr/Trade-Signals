...
def analyze_instrument(oanda_symbol):
    ...
    results["swing_high"] = swing_high if 'swing_high' in locals() else None
    results["swing_low"] = swing_low if 'swing_low' in locals() else None
...
# ---------- Bias & Reversal helpers (update 1) ----------
...
def reversal_sniper(entry, df_15m, df_5m, df_1m, bias, instrument):
    ...
    Now: only 1x 1m candle confirmation (instant entry for High Confidence setups).
    """
    ...
    # last single closed 1m candle direction
    one_up = one_down = False
    try:
        if len(df_1m) >= 2:
    ...
    if (bias == "LONG" and dir5 == "BUY") or (bias == "SHORT" and dir5 == "SELL"):
        score += 1
    if (bias == "LONG" and one_up) or (bias == "SHORT" and one_down):
        score += 1
    ...
    # --- Entry logic ---
    allow_instant = (label == "High")  # instant entry for high confidence
    if bias == "LONG":
        if dist_low <= tol and (allow_instant or one_up) and (label=="High" or dir5=="BUY") and dir1=="BUY":
        ...
        if dist_high <= tol and (allow_instant or one_down) and (label=="High" or dir5=="SELL") and dir1=="SELL":
            buffer = max(abs(entry)*0.002, 1.0)
